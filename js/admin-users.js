...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function initializeModals() {
        ...
        nextBtn?.addEventListener('click', async () => {
            if (state.currentStep === 1) {
                ...
                if (!document.getElementById('parentUsername').value || !document.getElementById('parentPassword').value) {
    ...
    async function submitStaffRegistration() {
        ...
        try {
            ...
            // 2. Create Role Entry
            if (role === 'teacher') {
                await supabase.from('teachers').insert([{ id: profile.id, employee_no: employeeID || staffID }]);
            } else if (role === 'clinic') {
                await supabase.from('clinic_staff').insert([{ id: profile.id }]);
            } else if (role === 'guard') {
                await supabase.from('guards').insert([{ id: profile.id }]);
            }

            utils.showNotification(`Staff account created successfully! ID: ${staffID}`, 'success');
    ...
    async function submitParentStudent() {
        ...
        try {
            ...
            if (pError) {
                ...
                throw new Error(`Profile creation failed: ${pError.message}`);
            }
    ...
    async function submitEditUser() {
        ...
        try {
            ...
            const updateData = { full_name, phone, email, is_active, photo_url: photoUrl };
            if (password) updateData.password = password;

            // 1. Update Profile
            const { error: pError } = await supabase.from('profiles').update(updateData).eq('id', id);
            if (pError) throw pError;

document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.saveFindings = async () => {
        ...
        try {
            ...
            // 2. Update attendance remarks with decision
            // Need to find the latest clinic attendance for this student
            const { data: attendance } = await supabase
                .from('attendance')
                .select('id, recorded_by')
                .eq('student_id', state.selectedPatient.students.id)
            ...
            if (attendance) {
                await supabase
                    .from('attendance')
                    .update({ remarks: `Decision: ${state.currentDecision} | Nurse Note: ${notes}` })
                    .eq('id', attendance.id);

                // 3. Notify Teacher (Decision Phase)
                await supabase.from('notifications').insert([{
                    recipient_id: attendance.recorded_by,

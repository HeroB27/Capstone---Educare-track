...
document.addEventListener('DOMContentLoaded', () => {
    utils.checkAccess(['guard', 'teacher', 'admin']);
    ...
    async function onScanSuccess(text) {
        ...
        try {
            ...
            const entryType = current === 'out' ? 'entry' : 'exit';
            const status = deriveStatus(entryType);
            const remarks = deriveRemarks(entryType, status);
            const tsISO = new Date().toISOString();
            const { error: aError } = await supabase.from('attendance').insert([{
            ...
            await supabase.from('students').update({ current_status: entryType === 'entry' ? 'in' : 'out' }).eq('id', student.id);
            ...
            if (parentId) {
                await supabase.from('notifications').insert([{
                    ...
                    object: { student_id: student.id, full_name: student.full_name, remarks, time: tsISO }
                }]);
            }
            renderStatus(student.full_name, entryType, remarks);

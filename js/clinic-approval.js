    ...
    async function fetchPendingPasses() {
        try {
            const { data, error } = await supabase
                .from('attendance')
                .select('*, students(full_name, grade_level, strand), profiles:recorded_by(full_name)')
            ...
            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300">
                <div class="flex justify-between items-start mb-6">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-xs font-bold text-gray-900">${utils.formatTime(p.timestamp)}</p>
                    </div>
                </div>

                <div class="bg-gray-50 rounded-2xl p-4 mb-6">
                    <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-2">Teacher's Note</p>
                    <p class="text-sm text-gray-700 font-medium leading-relaxed italic">"${p.remarks}"</p>

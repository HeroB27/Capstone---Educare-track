...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('submitPass')?.addEventListener('click', async () => {
        ...
        try {
            ...
            // 1. Create Clinic Pass (Pending Approval)
    ...
    async function fetchPasses() {
        try {
            const [passRes, visitRes] = await Promise.all([
                supabase
                    .from('clinic_passes')
                    .select('*, students(id, full_name, grade_level, strand), clinic_visits(id, visit_time, reason, notes, status)')
                    .eq('issued_by', state.user.id)
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">${utils.formatDate(p.issued_at)} at ${utils.formatTime(p.issued_at)}</p>
                        </div>
                    </div>
                    <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        p.status === 'used' || p.status === 'rejected' ? 'bg-gray-100 text-gray-600' :
                        p.status === 'approved' ? 'bg-emerald-100 text-emerald-700' :
                        p.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-amber-100 text-amber-700'
                    }">${p.status}</span>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 pt-4 border-t border-gray-50">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 italic">"${p.reason || 'No details'}"</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-gray-400 font-bold uppercase mb-2">Current Location</p>
                        <div class="flex items-center space-x-2">
                            <div class="w-2 h-2 rounded-full ${p.clinic_visit_id ? 'bg-blue-500 animate-pulse' : 'bg-gray-300'}"></div>
                            <span class="text-xs font-bold text-gray-700 uppercase">${p.clinic_visit_id ? 'In Clinic' : 'Outside'}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function renderFindingsApproval() {
        ...
        const pendingApprovals = state.passes
            .filter(p => p.clinic_visits?.notes?.includes('Decision:') && !p.clinic_visits?.notes?.includes('Parent Notified'));
        ...
        list.innerHTML = pendingApprovals.map(p => {
            const decisionMatch = p.clinic_visits.notes.match(/Decision: (\w+)/);
            ...
            return `
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group">
                    ...
                    <button onclick="approveFindings('${p.clinic_visit_id}', '${p.id}')" 
    ...
    window.approveFindings = async (visitId, passId) => {
        try {
            utils.showNotification('Approving and notifying parent...', 'info');
            
            const pass = state.passes.find(p => p.id === passId);
            if (!pass?.clinic_visits) throw new Error('Clinic visit not found for this pass');
            ...
            if (parentLink) {
                ...
                await supabase.from('notifications').insert([{
                    ...
                    object: { 
                        ...
                        details: pass.clinic_visits.notes 
                    }
                }]);
            }
            ...
            // Mark as "notified" by updating clinic visit notes
            await supabase
                .from('clinic_visits')
                .update({ notes: pass.clinic_visits.notes + ' | Parent Notified' })
                .eq('id', visitId);
    ...
    function updateStats() {
        const active = state.passes.filter(p => p.status === 'pending' || p.status === 'approved').length;
